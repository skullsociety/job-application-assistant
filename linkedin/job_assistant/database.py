...
class JobRepository:
    ...
    def upsert(self, job: Job) -> Job:
        ...
        self.connection.execute(
            ...
            ON CONFLICT(url) DO UPDATE SET
              ...
              notes=COALESCE(jobs.notes, excluded.notes), updated_at=CURRENT_TIMESTAMP
            """,
            (
        ...
        self.connection.execute(
            """UPDATE jobs SET closing_date=COALESCE(?, closing_date),
               key_skills=COALESCE(?, key_skills), source_job_id=COALESCE(?, source_job_id),
               description=job_description, captured_at=COALESCE(captured_at, first_seen_at, created_at)
               WHERE url=?""",
            (job.closing_date, job.key_skills, job.source_job_id, job.url),
        )
    ...
    def update(self, job_id: int, *, score: int | None, notes: str | None, status: str | None, follow_up_date: str | None = None) -> Job:
        ...
        if follow_up_date is not None:
            self.set_followed_up(job_id, bool(follow_up_date))
        self.connection.commit()
    ...
    def set_followed_up(self, job_id: int, followed_up: bool) -> Job:
        """Compatibility entry point: follow-up state is derived from its date."""
        job = self.get(job_id)
        if not job:
            raise KeyError(f"No job found with id {job_id}.")
        followed_up = bool(job.follow_up_date)
    ...
    def save_match(self, job_id: int, *, score: int, matching_skills: str, missing_skills: str, reason: str, recommendation: str, expected_hash: str | None = None) -> Job | None:
        job = self.get(job_id)
        if not job:
            raise KeyError(f"No job found with id {job_id}.")
        cursor = self.connection.execute(
            """UPDATE jobs SET match_score=?, matching_skills=?, missing_skills=?, match_reason=?, recommendation=?,
               updated_at=CURRENT_TIMESTAMP WHERE id=? AND (? IS NULL OR description_hash=?)""",
            (score, matching_skills, missing_skills, reason, recommendation, job_id, expected_hash, expected_hash),
        )
        self.connection.commit()
    ...
    def set_tailored_resume_path(self, job_id: int, path: str | None, *, expected_hash: str | None = None) -> Job:
        """Record the locally generated PDF associated with a current match result."""
        job = self.get(job_id)
        if not job:
            raise KeyError(f"No job found with id {job_id}.")
        self.connection.execute(
            "UPDATE jobs SET tailored_resume_path=?, updated_at=CURRENT_TIMESTAMP WHERE id=? AND (? IS NULL OR description_hash=?)",
            (path, job_id, expected_hash, expected_hash),
        )
        self.connection.commit()
        return self._required_job(job_id)
