...
test("job capture uses a current-id card or a verified document-title detail pane", () => {
  ...
  assert.match(script, /function extractHeaderPreview\(\)/);
  assert.match(script, /\[class\*='JobDetails_AboutTheJob'\]/);
  assert.match(script, /stop\.has\(line\.toLowerCase\(\)\) && description\.length/);
...
test("side panel re-reads a listing before capture and after tab navigation", () => {
  ...
  assert.match(script, /EXTRACT_LINKEDIN_HEADER/);
});
