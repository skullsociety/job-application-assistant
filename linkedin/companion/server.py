...
class BackgroundProcessor:
    ...
    def enqueue_all(self) -> int:
        jobs = [job for job in self.store.list() if job.source == "linkedin"]
        for job in jobs:
    ...
    def do_POST(self) -> None:  # noqa: N802 - standard library handler hook
        ...
        try:
            ...
            if self.path == "/api/jobs/clear":
                raise PermissionError("Bulk deletion is disabled for the shared tracker. Delete individual rows from the common dashboard.")
