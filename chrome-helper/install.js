...
function stableId(folder) {
  const manifest = JSON.parse(fs.readFileSync(path.join(ROOT, folder, "extension/manifest.json"), "utf8"));
...
function install() {
  ...
  const ids = [stableId("linkedin"), stableId("jobstreet"), stableId("careersgov")];
  ...
  fs.writeFileSync(manifest, JSON.stringify({name: NAME, description: "Job assistants: Chrome-managed local startup and shutdown", path: launcher,
    type: "stdio", allowed_origins: ids.map(id => `chrome-extension://${id}/`)}, null, 2));
  ...
  console.log("Shared Chrome helper installed for these extension IDs: " + ids.join(", "));
  console.log("Reload all three extensions. Close old companion windows once; future helpers start and stop with Chrome.");
}
