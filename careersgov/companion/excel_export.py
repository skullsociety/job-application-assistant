...
from shared_tracker.schema import EXPORT_PATH as TRACKER_PATH
HEADERS = [
...
DETAIL_FIELDS = [
    ("ID", "id"), ("Role", "title"), ("Job URL", "url"), ("Employment Type", "employment_type"),
...
def export_jobs(jobs: list[dict[str, Any]], output: Path = TRACKER_PATH) -> Path:
    ...
    sheet["A1"] = "Careers@Gov Job Application Tracker"
    ...
    for job in jobs:
        ...
        _append_text_safe(sheet, [
            _as_date(job.get("date_found") or job.get("captured_at")), "Careers@Gov", job["company"],
    ...
    detail = book.create_sheet("CareersGov Details")
    detail.append([label for label, _ in DETAIL_FIELDS])
    for job in unique:
        _append_text_safe(detail, [job.get(key) for _, key in DETAIL_FIELDS])
    _style_sheet(detail, 1, "CareersGovDetails", [10, 32, 48] + [28] * (len(DETAIL_FIELDS) - 3))
