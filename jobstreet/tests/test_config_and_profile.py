...
class ConfigAndProfileTests(unittest.TestCase):
    ...
    def test_config_uses_a_resume_folder_by_default(self) -> None:
        with patch.dict(os.environ, {"JOBSTREET_SHARE_LINKEDIN_TRACKER": "false"}, clear=True):
            settings = get_settings(ROOT / "missing-config.toml")
    ...
    def test_sibling_linkedin_database_is_shared_by_default(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            parent = Path(directory).resolve()
            runtime_root = parent / "jobstreet"
            linkedin_root = parent / "linkedin"
            runtime_root.mkdir()
            (linkedin_root / "data").mkdir(parents=True)
            (linkedin_root / "resumes").mkdir()
            database = linkedin_root / "data" / "jobs.sqlite3"
            database.touch()
            with patch.dict(os.environ, {"JOB_ASSISTANT_HOME": str(runtime_root)}, clear=True):
                settings = get_settings()
            self.assertTrue(settings.shared_with_linkedin)
            self.assertEqual(settings.database_path, database)
            self.assertEqual(settings.export_path, linkedin_root / "exports" / "job_tracker.xlsx")
            self.assertEqual(settings.resume_dir, linkedin_root / "resumes")

    def test_linkedin_sharing_can_be_disabled(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            parent = Path(directory).resolve()
            runtime_root = parent / "jobstreet"
            linkedin_root = parent / "linkedin"
            runtime_root.mkdir()
            (linkedin_root / "data").mkdir(parents=True)
            (linkedin_root / "data" / "jobs.sqlite3").touch()
            environment = {
                "JOB_ASSISTANT_HOME": str(runtime_root),
                "JOBSTREET_SHARE_LINKEDIN_TRACKER": "false",
            }
            with patch.dict(os.environ, environment, clear=True):
                settings = get_settings()
            self.assertFalse(settings.shared_with_linkedin)
            self.assertEqual(settings.database_path, runtime_root / "data" / "jobs.sqlite3")
