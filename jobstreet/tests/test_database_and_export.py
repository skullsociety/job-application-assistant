...
class DatabaseAndExportTests(unittest.TestCase):
    ...
    def test_save_and_export_job(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            repository.update(saved.id or 0, score=None, notes=None, status=None, follow_up_date='2026-09-10')
    ...
    def test_follow_up_status_and_events_are_preserved(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            with JobRepository(Path(directory) / "jobs.sqlite3") as repository:
                ...
                self.assertFalse(repository.set_followed_up(saved.id or 0, True).followed_up)
                followed_up = repository.update(saved.id or 0, score=None, notes=None, status=None, follow_up_date='2026-09-10')
                ...
                self.assertTrue(repository.set_followed_up(saved.id or 0, False).followed_up)
                unmarked = repository.update(saved.id or 0, score=None, notes=None, status=None, follow_up_date='')
