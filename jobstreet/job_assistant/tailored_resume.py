...
def create_tailored_resume(resume_text: str, job: Job, match: MatchResult, output_dir: Path) -> Path:
    ...
    if job.id is None:
        raise ValueError("Save the job before creating its tailored resume so the filename has a job ID.")
    output_dir.mkdir(parents=True, exist_ok=True)
    output = output_dir / f"{job.id}_tailored_resume_{_safe_name(job.company)}_{_safe_name(job.title)}.pdf"
