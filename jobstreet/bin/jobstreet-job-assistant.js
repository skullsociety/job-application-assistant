...
const TASK_NAME = "JobStreet Job Application Assistant";
const NATIVE_HOST_NAME = "com.jobstreet_job_assistant.launcher";
...
function defaultRuntimeRoot() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.LOCALAPPDATA || process.env.APPDATA || os.homedir(), "JobStreetJobAssistant");
  }
  if (process.platform === "darwin") return path.join(os.homedir(), "Library", "Application Support", "JobStreetJobAssistant");
  return path.join(process.env.XDG_DATA_HOME || path.join(os.homedir(), ".local", "share"), "jobstreet-job-assistant");
}
...
function writeNativeHostFiles() {
  ...
  const manifest = {
    ...
    description: "Starts the local JobStreet Job Application Assistant companion when Chrome requests it.",
    path: launcher,
    type: "stdio",
...
function usage() {
  console.log(`Usage: jobstreet-job-assistant <command>
