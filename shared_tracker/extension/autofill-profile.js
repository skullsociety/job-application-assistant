...
const resumeScalarKeys = new Set(['skills']);
...
function render(data) {
  ...
  for (const [key,label] of Object.entries(scalarFields)) {
    ...
    const fromResume = resumeScalarKeys.has(key);
    ...
    tools.append(hint,reset); wrapper.appendChild(tools); (fromResume ? resumeHost : manualHost).appendChild(wrapper);
  }
...
async function load(force = false) {
  ...
  const data = await Promise.race([
    chrome.runtime.sendMessage({type:'GET_SF_PROFILE'}),
    new Promise((_, reject) => setTimeout(() => reject(new Error('The local assistant did not respond within 15 seconds. Run Setup Job Assistant Suite.bat, then reload this extension.')), 15000)),
  ]);
...
render({ok:false,enabled:true,revision:'waiting',defaults:{education:[],employment:[]},overrides:{},custom_answers:[],source:null,warnings:[],profile:{education:[],employment:[]}});
status.textContent = 'Connecting to the local assistant…';
load().catch(error => { status.textContent = error.message; document.querySelector('#source').textContent = 'Local assistant unavailable.'; });
