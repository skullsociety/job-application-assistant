...
from .tracking import VALID_STATUSES
...
def render(jobs: list[dict[str, Any]], revision: str) -> str:
    ...
    for job in jobs:
        ...
        resume_link = f'<a href="/api/jobs/{identifier}/resume" target="_blank" rel="noreferrer">Open tailored PDF</a>' if resume else "—"
        ...
        hidden_fields = ("source", "source_job_id", "legacy_careersgov_id", "company_url", "application_url",
                         "application_method", "seniority_level", "applicant_count", "resume_name", "description_hash",
                         "last_seen_at", "created_at", "updated_at", "priority", "interest_level", "tags")
        detail_metadata = "".join(f"<p><b>{e(key.replace('_', ' ').title())}:</b> {e(job.get(key) if job.get(key) is not None else 'Not shown')}</p>" for key in hidden_fields)
        rows.append(
            ...
            f'<td data-sort="{e(job.get("follow_up_date"))}"><input class="follow-up-date" data-id="{identifier}" data-saved-value="{e(job.get("follow_up_date"))}" type="date" min="2000-01-01" aria-label="Follow-up date for {title}" value="{e(job.get("follow_up_date"))}"></td>'
    ...
    columns = [("Position", "text"), ("Captured", "date"), ("Match", "number"), ("Applied?", "number"), ("Follow-up date", "date"), ("Resume", "number"), ("Closing", "date")]
