const state = { preview: null, saved: null, timer: 0, listingTimer: 0 };
const elements = {
...
async function extractHeaderPreview() {
  const response = await sendToActive({ type: "EXTRACT_LINKEDIN_HEADER" });
  if (!response?.ok) throw new Error(response?.error || "The current job header is not visible yet.");
  elements.title.textContent = response.job.title;
  elements.company.textContent = response.job.company;
  elements.location.textContent = response.job.location || "Location not shown";
  elements.workplace.textContent = response.job.workplace_type || "Workplace not shown";
  elements.posted.textContent = response.job.posting_date || "Posting date not shown";
  setStatus("Current listing updated. Scroll to About the job and expand it before capturing.");
}
...
elements.capture.addEventListener("click", async () => {
  ...
  try {
    // LinkedIn changes listings in-place. Always read again so this button
    // cannot save the listing that happened to be visible when the panel opened.
...
function scheduleListingRefresh(tabId) {
  clearTimeout(state.listingTimer);
  state.listingTimer = setTimeout(async () => {
    try {
      const tab = await activeTab();
      if (tab.id !== tabId || !isLinkedInJob(tab)) return;
      state.preview = null;
      elements.title.textContent = "Reading current listing…";
      elements.company.textContent = "";
      elements.location.textContent = "";
      elements.workplace.textContent = "";
      elements.posted.textContent = "";
      await extractHeaderPreview();
    } catch (_) {
      // The page can be incomplete while LinkedIn navigates in place.
      // Capture still performs a fresh read when the user clicks it.
    }
  }, 500);
}

chrome.tabs.onUpdated.addListener((tabId, change) => {
  if (change.url || change.status === "complete") scheduleListingRefresh(tabId);
});
chrome.tabs.onActivated.addListener(({ tabId }) => scheduleListingRefresh(tabId));
