(() => {
  "use strict";
  // SuccessFactors is handled by the shared resume-driven autofill engine.
  if (location.hostname.endsWith('.successfactors.com')) return;
  ...
  function aboutText() {
    ...
    for (const line of lines.slice(start + 1)) {
      // Some employers begin the description itself with “About the Company”.
      // Only treat a section heading as the end after actual job text was read.
      if (stop.has(line.toLowerCase()) && description.length) break;
  ...
  function extractJob() {
    const preview = extractHeaderPreview();
  ...
  chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    try {
      ...
      if (message?.type === "EXTRACT_LINKEDIN_HEADER") {
        sendResponse({ ok: true, job: extractHeaderPreview() });
        return false;
      }
